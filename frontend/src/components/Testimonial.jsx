    ...
    <div className="w-full bg-gray-50 py-16">
      ...
      <div className="text-center mb-12">
        <div className="flex items-center justify-center gap-2 mb-3">
          ...
          <span className="text-sm uppercase tracking-wider text-pink-400 font-medium">Love Stories</span>
          <HeartIcon className="w-6 h-6 text-pink-400" />
        </div>
        <h2 className="text-4xl font-serif font-medium mb-4">What Our Brides Say</h2>
              ...
              {Array.from({ length: totalSlides }).map((_, slideIndex) => (
                <div key={slideIndex} className="w-full flex-shrink-0 flex gap-4">
                  {testimonials.slice(slideIndex * 3, slideIndex * 3 + 3).map((testimonial) => (
                    <div
                      key={testimonial.id}
                      className="w-1/3 p-6 bg-white rounded-lg shadow-lg"
                    >
