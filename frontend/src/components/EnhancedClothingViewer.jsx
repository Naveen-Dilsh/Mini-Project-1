...
const EnhancedClothingViewer = ({ gender = "male", pose = "front", showAccessories = true }) => {
  ...
  const handlePositionSave = async (clothingId, position) => {
    try {
      await updateMaterialPositioning(clothingId, editingClothing.type, position)
  ...
  const renderClothingLayer = (layer) => {
    ...
    if (!clothing || !layerVisibility[layer.type]) return null

    // Get positioning data
    const positioning = clothing.positioning?.[layer.type] || {
      x: 0,
      y: layer.type === "trousers" ? 20 : layer.type === "jacket" ? -10 : 0,
      scale: layer.type === "jacket" ? 1.05 : 1,
      rotation: 0,
      opacity: 1,
    }
    ...
    return (
      <motion.div
        ...
        style={{
          zIndex: layer.zIndex,
          transform: `translate(${positioning.x}px, ${positioning.y}px) scale(${positioning.scale}) rotate(${positioning.rotation}deg)`,
      ...
      >
        <img
          ...
          style={{
            ...
            filter: layer.type === "trousers" ? "brightness(0.95)" : "none", // Slightly darken trousers
          }}
  ...
  return (
    <div className="relative w-full h-full bg-gradient-to-b from-gray-100 to-gray-200 rounded-lg overflow-hidden">
      {/* Debug Controls */}
      {debugMode && (
        <div className="absolute top-4 left-4 z-50 bg-white rounded-lg p-3 shadow-lg">
          <h4 className="font-semibold text-sm mb-2">Layer Controls</h4>
          <div className="space-y-1">
            <label className="flex items-center text-xs">
              <input
                type="checkbox"
                checked={layerVisibility.base}
                onChange={() => toggleLayerVisibility("base")}
                className="mr-2"
              />
              Base Character
            </label>
            {clothingLayers.map((layer) => (
              <label key={layer.type} className="flex items-center text-xs">
                <input
                  type="checkbox"
                  checked={layerVisibility[layer.type]}
                  onChange={() => toggleLayerVisibility(layer.type)}
                  className="mr-2"
                />
                <span className="capitalize">{layer.type}</span>
                {currentClothing[layer.type] && <span className="ml-1 text-green-600">✓</span>}
                {currentClothing[layer.type] && (
                  <button
                    onClick={() => setEditingClothing({ ...currentClothing[layer.type], type: layer.type })}
        ...
        <div className="bg-white bg-opacity-90 rounded-lg p-3 shadow-lg">
          <div className="grid grid-cols-2 gap-2 text-xs">
            {Object.entries(currentClothing).map(([type, clothing]) => {
              ...
              const positioning = clothing.positioning?.[type]
              ...
              return (
                <div key={type} className="flex items-center space-x-2">
                  ...
                  {positioning && <span className="text-blue-500 text-xs">📐</span>}
                </div>
              )
            })}
          </div>
...
export default EnhancedClothingViewer
